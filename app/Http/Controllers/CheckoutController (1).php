...
class CheckoutController extends Controller
{
    ...
    {
		// $user_id = intval(base64_decode($request->input("user_id")));
		// $sku = base64_decode($request->input("sku"));
		// $amount = intval(base64_decode($request->input("amount")));
		// $plan = base64_decode($request->input("plan"));
		// $type = base64_decode($request->input("type"));
		// $box_product_id = base64_decode($request->input("box_product_id"));
		// dd($user_id,$sku,$amount,$plan,$type,$box_product_id);
		//$type = $request->input("type");

		$type = isset($type) && !empty($type)? $type: $request->input("type");
		...
		if(isset($type) && $type==1){
			//books
			$api_amount = intval(base64_decode($request->input("amount")));
			$amount = isset($api_amount) && !empty($api_amount)? $api_amount: $request->input("amount");

			$data["txnid"] = "Txn" . rand(10000,99999999);
			$data["amount"] = $amount;// can use plan price or total book price here i.e $total
